from typing import Tuple, List, TypeVar, Union, Sequence
...
class Node:
  def __init__(self, curr: Coordinate, end: Coordinate, parent_node: Union["Node", None]) -> None:
...
def find_shortest_path(start: Coordinate, end: Coordinate, obstacles: List[Coordinate]) -> List[Coordinate]:
  ...
  while(True):
    ...
    for edge in edges:
      new_coord: Coordinate = (curr_node.coord[0] + edge[0], curr_node.coord[1] + edge[1])
      new_node: Node = Node(new_coord, end, curr_node)

      # obstacles 혹은 closed_list에 존재하는 node는 추가하지 않음.
      if find(new_coord, obstacles) != -1 or find(new_node, closed_list, True) != -1:
        continue
